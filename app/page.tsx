...
export default function HomePage() {
  ...
  const pillars = [
    {
      icon: "⬡",
      title: p.governance.title,
      description: p.governance.description,
      accent: "var(--accent)",
    },
    {
      icon: "◈",
      title: p.visualization.title,
      description: p.visualization.description,
      accent: "var(--accent2)",
    },
    {
      icon: "⟳",
      title: p.automation.title,
      description: p.automation.description,
      accent: "var(--accent3)",
    },
  ];
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 items-center">
            ...
            <div className="lg:col-span-3">
              ...
              >
                {h.headline_1}{" "}
                <span className="gradient-text">{h.headline_accent}</span>{" "}
                {h.headline_2}
              </motion.h1>
            ...
            <div className="lg:col-span-2 space-y-4">
              <FloatingCard
                icon="✈"
                title={t.floating.last_mission}
                value="Air France — BlueBiz Platform"
                delay={0.5}
              />
              <FloatingCard
                icon="⚡"
                title={t.floating.stack}
                value="Python · SQL · Next.js · Power BI"
                delay={0.65}
              />
              <FloatingCard
                icon="◎"
                title={t.floating.availability}
                value={t.floating.availability_value}
                delay={0.8}
              />
            </div>
          </div>
            ...
            {pillars.map((pillar, index) => (
              <SectionReveal key={index} delay={index * 0.1}>
                ...
                >
                  ...
                  <h3 className="font-epilogue font-bold text-lg text-[var(--text)] mb-3 relative z-10">
                    {pillar.title}
                  </h3>
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <SectionReveal>
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-12">
              ...
              <Link
                href="/projects"
                className="text-sm font-mono text-[var(--muted)] hover:text-[var(--accent)] transition-colors duration-200 flex items-center gap-1.5"
              >
                {t.projects_section.all} <span>→</span>
              </Link>
            </div>
          </SectionReveal>
